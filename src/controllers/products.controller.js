...
export const searchProductsByType = async (req, res) => {
    ...
    if (!idTypeProduct) return res.status(400).json({ message: "Por favor, rellene todos los campos" })

    try {
        const products = await Product.find({ idTypeProduct: idTypeProduct }).populate('idTypeProduct', 'nameTypeProduct');
        if (products.length === 0) return res.status(404).json({ message: "No hay productos para este tipo" });
